...
if gen_data_:
    ...
    snap_info_quiet.drop(columns=["skycoord","mosaic_coord"]).to_pickle(data_folder + f'snap_info_quiet_{global_size}.pkl')
    snap_info_loud.drop(columns=["skycoord","mosaic_coord"]).to_pickle(data_folder + f'snap_info_loud_{global_size}.pkl')

else:
    snap_info_quiet = pd.read_pickle(data_folder + f'snap_info_quiet_{global_size}.pkl')
    
    snap_info_loud = pd.read_pickle(data_folder + f'snap_info_loud_{global_size}.pkl')      
...
if False:
    ...
    with open(f'{n_}_random_n_points.npy', 'wb') as f:
        np.save(f, random_snapshots)
else:  
    with open(f'{n_}_random_n_points.npy', 'rb') as f:
        random_snapshots = np.load(f)
random_snapshots = random_snapshots.reshape(n_)

if False:
    plt.hist(1000*random_snapshots, bins=np.linspace(np.min(1000*random_snapshots), np.max(1000*random_snapshots), 100))
    plt.title("Histogram of Monte Carlo simulated mean fluxes")
    plt.xlabel("mJy/beam"); plt.ylabel("Count")
    plt.show()
sample_std = np.std(random_snapshots)
detection_sigma = stack_data[global_size-1][global_size-1]/sample_std


##One single (long) random sample to compare against mean and std
...
if True:
    ...
    random_coord_df = snapshots(pd.DataFrame(random_coord_dict), mosaic_active, global_size)
    ...
    if True:
        ...
        visualise(random_coord_stack, title="Stacked random points in the mosaics", save="random_stack.png")

@verbose_iterate_to_array(1e6, chunk=1000)
def single_iterated_random(*args,**kwargs):
    return random_stack(*args,**kwargs)

if gen_data_:
    rng = np.random.default_rng(123)
    long_data_mjy = single_iterated_random(rng, mosaic_active, len_=1, s=1).flatten()*1000
    long_data_mjy_df = pd.DataFrame(long_data_mjy)
    long_data_mjy_df.to_csv(data_folder + 'long_data_mjy.csv')
else:
    long_data_mjy_df = pd.read_csv(data_folder + 'long_data_mjy.csv')
    long_data_mjy = long_data_mjy_df.values.flatten()[1::2]

if True:
    data_mjy = long_data_mjy.flatten()
    mu = np.mean(data_mjy)
    sig = np.std(data_mjy)
    ns, bins, patchs = plt.hist(data_mjy, bins=np.linspace(mu-10*sig,mu+10*sig,num=100), log=1)
    plt.axvline(x=mu, c="red")
    plt.xlabel("mJy/beam")
    plt.ylabel("Count (log)")
    plt.title("Histogram of flux values for random points in the combined mosaic")
    plt.savefig("noise_profile.png")
    plt.show()
    
    data_mjy_narrow = data_mjy[np.abs(data_mjy)<=10]
    better_mu = np.mean(data_mjy_narrow)
    better_sig = np.std(data_mjy_narrow)
    ns, bins, patchs = plt.hist(data_mjy, bins=np.linspace(better_mu-10*better_sig,better_mu+10*better_sig,num=500), log=0)
    plt.axvline(x=better_mu, c="red")
    x = np.linspace(better_mu-10*better_sig,better_mu+10*better_sig,num=500)
    y = norm.pdf(x,better_mu,better_sig)
    plt.plot(x,y*(np.max(ns)/np.max(y)))
    plt.xlabel("mJy/beam")
    plt.ylabel("Count")
    plt.title("Gaussian noise profile at low noise values")
    plt.savefig("gaussian_noise_profile.png")
    #plt.ylim([1,1e6])
    plt.show()
